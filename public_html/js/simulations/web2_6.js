...
export const web2_6 = () => {
    ...
    $('#configForm').addEventListener('submit', function (event) {
        ...
        function drawParticleCounter() {
            ...
            ctx.fillText(`Web 2.6 Users: ${particles.length}`, 10, 10); // Positioned in the top-left corner
        }
        ...
        class Particle {
            ...
            interact() {
                // Allow only initial particles to gain
                if (this.target && this.isInitial && Math.hypot(this.x - this.target.x, this.y - this.target.y) < this.size + this.target.size) {
                    // ... interaction logic for sharing and gaining ...
                    const sharePercentage = Math.random() * (config.shareRange.max - config.shareRange.min) + config.shareRange.min;
        ...
        for (let i = 0; i < config.initialParticleSizes.length; i++) {
            ...
            particles.push(new Particle(initialSize, true)); // true for initial particles
        }

        function addNewParticle() {
            if (particles.length < config.maxParticles) {
                const size = Math.random() * (config.sizeRange.max - config.sizeRange.min) + config.sizeRange.min;
                const newParticle = new Particle(size, false); // false for non-initial particles
